...
//                 "SELECT product_id FROM cart WHERE user_id = ?",sessionStorage.id
//             );
//             const result: OrderItem[] = await queryDatabase<OrderItem[]>(
//                 "SELECT id, title, thumbnail, images, description, authors, tags, price FROM product WHERE id = ?",cartIdReader
//             );
//             res.json(result);
//         } catch (error: any) {
//             console.error("Database Error:", error);
